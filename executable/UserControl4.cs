...
namespace CubesatProject
{
    ...
    {
        MySqlConnection connection = new MySqlConnection("server=2600:6c50:487f:ca92:7815:45b7:a9a5:b2e6;port=3308;username=root;password=;database=cubesat_login");//Connection for the database
        ...
        private void confirmEmailButton_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                Boolean acceptable = emailValidation(this.textBox1.Text);//Checks if email in valid format
                if (acceptable)
                {

                    valid = ValidateEmailandPass(oldEmail, this.passwordEmailBox.Text);//Checks if credentials are correct
                    if (!valid)
                        this.incorrectPasswordLabel.Visible = true;
                    else
                    {
                        if ((this.textBox1.Text.Equals(oldEmail)) != true)
                        {
                            ...
                            {
                                MessageBox.Show("Email could not be updated...");
                            }
                        }
                        else
                        {
                            this.emailDifferentLabel.Visible = true;
                        }
